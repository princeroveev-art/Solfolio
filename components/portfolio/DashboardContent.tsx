...
export function DashboardContent() {
  ...
  const { solBalance, solPrice, quoteMeta, tokens, transactions, loading, error } = usePortfolioData(publicKey, connected, connection);
  const { totalUSD, grindScore, agentReport } = usePortfolioAnalytics(solBalance, solPrice, quoteMeta, tokens, transactions);
  ...
  const statCards = [
    ...
    { label: "SOL Position", value: loading ? null : `${solBalance.toFixed(3)} SOL`, sub: loading ? "" : formatUSD(solBalance * solPrice), Icon: SolIcon },
  ...
  return (
    ...
    >
      ...
      {!loading && quoteMeta.fetchedAt !== new Date(0).toISOString() && (
        <div style={{ color: "var(--color-text-muted)", fontSize: "var(--text-xs)", marginTop: "-8px" }}>
          Quotes: {quoteMeta.source} · {new Date(quoteMeta.fetchedAt).toLocaleTimeString()} {quoteMeta.isFallback ? "(fallback)" : ""}
        </div>
      )}
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }} className="lg:col-span-2">
          <TokenList tokens={tokens} solBalance={solBalance} solPrice={solPrice} loading={loading} />
