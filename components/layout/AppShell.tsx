          ...
          <div className="relative flex flex-col gap-4">
            <div className="flex flex-wrap items-center justify-between gap-3 lg:gap-4">
              <div className="flex items-center gap-3 min-w-0">
                <button className="sf-menu-btn" type="button" onClick={() => setMobileOpen(true)} aria-label="Open navigation">Menu</button>
                <div className="rounded-2xl border border-white/15 bg-slate-950/50 px-4 sm:px-5 py-3.5 sm:py-4 min-w-[260px] sm:min-w-[320px]">
                  <div className="flex items-center gap-3.5 sm:gap-4">
                    <div className="flex h-11 w-11 sm:h-12 sm:w-12 items-center justify-center rounded-xl border border-violet-300/35 bg-gradient-to-br from-violet-500/30 via-indigo-500/20 to-sky-500/25 shadow-[0_8px_24px_rgba(79,70,229,0.28)]">
                      <span className="text-lg sm:text-xl font-semibold text-violet-100 tracking-tight">S</span>
                    </div>
                    <div className="min-w-0">
                      <p className="text-[11px] uppercase tracking-[0.26em] text-violet-200/90 leading-none">Solfolio</p>
                      <h1 className="mt-1 text-lg sm:text-xl font-semibold tracking-[-0.01em] text-white leading-tight">Intelligence Trading Terminal</h1>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
                ...
                <select value={network} onChange={(e) => setNetwork(e.target.value as Network)} className="rounded-xl border border-white/20 bg-slate-900/85 px-3.5 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-violet-400">
                ...
                <div className="relative">
                  <button
                    onClick={() => (connected ? setShowWalletMenu((v) => !v) : setVisible(true))}
                  ...
                  {showWalletMenu && (
                    <div className="absolute right-0 mt-2 w-80 rounded-2xl border border-white/15 bg-slate-950/95 p-3 shadow-2xl shadow-black/50 z-50">
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                        <div className="rounded-lg border border-white/10 bg-white/[0.04] p-2"><p className="text-slate-400">Portfolio</p><p className="text-slate-100 font-semibold">{walletSummary.tvl}</p></div>
                        <div className="rounded-lg border border-white/10 bg-white/[0.04] p-2"><p className="text-slate-400">24h PnL</p><p className="text-emerald-300 font-semibold">{walletSummary.pnl}</p></div>
                      </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_auto] gap-4 items-stretch">
              <label className="w-full group relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 text-sm">Search</span>
                <input type="text" placeholder="Search wallets, tokens, NFT collections, tx hashes" className="w-full rounded-2xl border border-white/20 bg-slate-900/85 pl-20 pr-24 py-4 text-base placeholder:text-slate-500 focus:outline-none focus:border-violet-300 focus:ring-2 focus:ring-violet-500/20" />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-300 border border-white/20 bg-white/5 rounded px-2 py-1">⌘K</span>
              </label>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {statusCards.map((item) => (
                  <div key={item.label} className="rounded-xl border border-white/15 bg-slate-900/70 px-4 py-3 min-w-[132px]">
